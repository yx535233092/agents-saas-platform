...
class ApplicationBase(BaseModel):
    ...
    app_type: str = Field(
        default="custom", description="应用类型：secret_judgement/custom"
    )
    api_endpoint: str = Field(
        ..., min_length=1, max_length=500, description="API端点地址"
    )
    ...
    timeout: Optional[int] = Field(
        default=300, ge=1, le=3600, description="超时时间（秒）"
    )
...
class ApplicationUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=200, description="应用名称"
    )
    description: Optional[str] = Field(None, description="应用描述")
    app_type: Optional[str] = Field(None, description="应用类型")
    api_endpoint: Optional[str] = Field(
        None, min_length=1, max_length=500, description="API端点地址"
    )
