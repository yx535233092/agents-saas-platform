...
# 决策评审节点
def get_decision_review_prompt():
    """获取决策评审的提示词模板"""
...
def decision_review_node(state, token_callback: Optional[Callable[[str], None]] = None):
    """
    决策评审节点（流式版本）

    Args:
        state: 工作流状态
        token_callback: 可选的回调函数，用于处理每个 token (token: str) -> None
...
# 正向涉密分析节点
def secret_analysis_node(state, token_callback: Optional[Callable[[str], None]] = None):
    """
    正向涉密分析节点（流式版本）
    ...
    try:
        ...
        evidence = (
            state.get("evidence", "")
            + f"节点{state.get('current_node', 'secret_analysis_node')}证据：{response_json['evidence']}"
        )
...
# 反向非涉密分析节点
def public_analysis_node(state, token_callback: Optional[Callable[[str], None]] = None):
    """
    反向非涉密分析节点（流式版本）
    ...
    try:
        ...
        evidence = (
            state.get("evidence", "")
            + f"节点{state.get('current_node', 'public_analysis_node')}证据：{response_json['evidence']}"
        )
        state.update({"evidence": evidence})
    except json.JSONDecodeError as e:
        print(f"{Fore.RED}JSON 解析失败: {str(e)}{Style.RESET_ALL}")
